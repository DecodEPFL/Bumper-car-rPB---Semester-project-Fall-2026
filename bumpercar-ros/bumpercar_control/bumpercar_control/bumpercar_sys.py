...
DEFAULT_DT = 1.0 / DEFAULT_RATE_HZ
KINEMATIC_MLP_MODEL = Path("mlp_simulation_models") / "model_kinematic_mlp.pth"


def resolve_kinematic_mlp_model_path():
    try:
        return str(Path(get_package_share_directory("bumpercar_sim")) / KINEMATIC_MLP_MODEL)
    except Exception:
        source_path = Path(__file__).resolve().parents[2] / "bumpercar_sim" / KINEMATIC_MLP_MODEL
...
class MLPDynamicsModel(nn.Module):
    def __init__(self,
        ...
        if model_path is None:
            model_path = resolve_kinematic_mlp_model_path()
    ...
    def pose_forward(self, car_state, delta_t=DEFAULT_DT):
    ...
    def velocity_forward(self, car_state, car_input, delta_t=DEFAULT_DT):
    # Unpack state
        vf      = car_state[:, 3]
        beta_f  = car_state[:, 4]
        beta_r  = car_state[:, 5]
        ...
        delta_dot = torch.clamp((delta_ref - delta) / self.par.tl_steering,
                                -self.par.steering_speed, self.par.steering_speed)
    ...
    def update(self, x_state, car_input, dt=DEFAULT_DT):
        # Update the state
