...
class MLPModel:
    def __init__(self,
        ...
        if model_path is None:
            model_path = os.path.join(
                get_package_share_directory("bumpercar_sim"),
                "mlp_sim_onnx_models",
                "model_kinematic_mlp.onnx",
            )
    ...
    def pose_forward(self, delta_t=1.0 / 25.0):
    ...
    def velocity_forward(self, car_input, delta_t=1.0 / 25.0):
        delta = self.car_state[6]
    ...
    def update(self, car_input, dt=1.0 / 25.0):
        self.prev_car_state = self.car_state.copy()

        next_pose = self.pose_forward(delta_t=dt)
        next_velocities = self.velocity_forward(car_input, delta_t=dt)
