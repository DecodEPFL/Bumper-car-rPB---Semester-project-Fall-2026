...
from algorithms import PositionPidController, NN_Controller, PolicyMLP
...
# initialize center car car
car0_state_init = np.array([5.0, 5.0, 0, 3.0, 0.1, 0.3, 0.5])
car0_state_final = np.array([5.0, 5.0, 0, 0, 0.0, 0.0, 0.0])
car_input_init = np.array([0.0, 0.0])
input_size = car_input_init.shape[0]
state_size = car0_state_init.shape[0]
cars_state_init = np.array([car0_state_init])
cars_state_final = np.array([car0_state_final])


# Initialize car states around x0
std_init = 10
std_final = 0 

INIT_TYPE = "GAUSSIAN" ## "GAUSSIAN" or "GRID"
if INIT_TYPE == "GAUSSIAN": 
    cars_state_init, cars_state_final = gaussianCarDistribution(car0_state_init, car0_state_final, std_init, std_final, car_number)
    cars_state = cars_state_init

elif INIT_TYPE == "GRID":
    cars_state_init, cars_state_final = create_grid_states(car0_state_final)
    cars_state = cars_state_init
...
cars_input_init = np.array([car_input_init])
...
nn_controller = NN_Controller(type="MLP", params=params)
nn_controller.load_weigths("sim/NN_controllers/MLP/policy_weights_1000cars_preTrain.pth")
...
for t in range(simulation_horizon):
    ...
    # Compute safe input
        if usePidController:
            ...
            for i in range(car_number):
                safe_input[i, :] = C3BFs[i].compute_safe_input(cars_state, cars_input[:, :, t], car0_state_final)
...
if plot_fig:
    if full_trajectory:
        fig, (ax_full, ax_dyn) = plt.subplots(
            nrows=1, ncols=2,
            figsize=(4.0, 2.0),
            dpi=200,
        )
    else:
        fig, ax_dyn = plt.subplots(
            nrows=1, ncols=1,
            figsize=(3, 3),
            dpi=250,
        )
    ...
    arrow_length = 0.10     # make this smaller/larger as you like
    dot_size = 0.1            # marker size
    traj_linewidth = 0.8
